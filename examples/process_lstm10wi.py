...
def train_test_split(sequence, n_steps, num_features, norm=True):
  ...
  train_seq, test_seq = sequence[int(0.2*data_size):, :], sequence[:int(0.2*data_size), :]

  if norm:
    mean, std = np.mean(train_seq, axis=0), np.std(train_seq, axis=0)
    train_seq = (train_seq - mean) / std
    test_seq = (test_seq - mean) / std
  else:
    mean, std = None, None

  X, y = split_sequence(sequence=train_seq, n_steps=n_steps)
  train_X = np.append(train_X, np.array(X), axis=0)
  train_y = np.append(train_y, np.array(y), axis=0)


  X, y = split_sequence(sequence=test_seq, n_steps=n_steps)
  test_X = np.append(test_X, np.array(X), axis=0)
  test_y = np.append(test_y, np.array(y), axis=0)

  return train_X, train_y, test_X, test_y, mean, std

def lstm_model(model_name, n_steps, X, y, epochs=100, cross_val=False, nfolds=10, run_train=False):
...
train_X, train_y, test_X, test_y, mean, std = train_test_split(dataset, n_steps, num_features)
...
for id, well in enumerate(uniq_well_code):
  ...
  sep_well_mean.append(output[4])
  sep_well_std.append(output[5])
...
metric2 = 100.0 * np.mean(np.abs(np.reshape(pred_y, [-1, 1]) - np.reshape(test_y, [-1, 1])) / (test_y + ymean / ystd))
...
for id, well in enumerate(uniq_well_code):
  ...
  well_mean = sep_well_mean[id]
  well_std = sep_well_std[id]
  metric2 = 100.0 * np.mean(np.abs(np.reshape(pred_y, [-1, 1]) - np.reshape(test_y, [-1, 1] )) / (test_y + well_mean[-1]/well_std[-1]))
  print('mse = %.3f '%(mse) , 'r2 = %.3f'%(r2), 'metric2 = %.2f'%(metric2))
