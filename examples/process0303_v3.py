...
def lstm_model(model_name, n_steps, X, y, run_train=False):
  ...
  if not run_train and os.path.isfile(os.path.join(dir, 'models', '%s_model.json' % (model_name))):
    ...
    folds = list(KFold(n_splits=10, shuffle=False, random_state=None).split(X, y))
    testScore = []
...
train_X, train_y, test_X, test_y = train_test_split(n_steps)
model = lstm_model(model_name='lstm10', X=train_X, y=train_y,
                   n_steps=n_steps, run_train=True)
