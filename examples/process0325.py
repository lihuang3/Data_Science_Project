...
def MLP_imput(dataset, idx_to_del, selected_features, imput_feature,
              target, test_idx, layers, dropout=0.0, epoch_num=100,
              test_ratio=0.1, verbose=0, display_plt=False, run_train=False):
  ...
  if not run_train and os.path.isfile( os.path.join(dir, 'models', '%s_model.json'%(imput_feature))):
    ...
    print('Starting training on %s ...'%(imput_feature))

    model = Sequential()
    model.add(Dense(layers[0], activation='relu', input_dim=num_features))
    if dropout>0.0:
    ...
    # fit model
    res = model.fit(train_X, train_y, validation_split=test_ratio, epochs=epoch_num, verbose=verbose)
    ...
    if display_plt:
      plt.plot(res.history['r2'])
      plt.plot(res.history['val_r2'])
      plt.plot(res.history['loss'])
      plt.plot(res.history['val_loss'])

      plt.ylabel('r2/loss')
      plt.xlabel('epoch')
      plt.legend(['train_r2', 'val_r2','train_loss', 'val_loss' ], loc='mid left')
      plt.show()

  pred_y = model.predict(val_X, verbose=0)
  print(imput_feature, '\n', 'val set r2: ', r2_score(val_y, pred_y))
...
MLP_imput(dataset, idx_to_del, selected_features=[3, 4, 5, 6, 7],
          imput_feature='AVG_DP_TUBING', target=8, test_idx=test_idx_dptb,
          layers=[16, 8], dropout=0.0, epoch_num=300, test_ratio=0.1,
          verbose=0, display_plt=False, run_train=False)

MLP_imput(dataset, idx_to_del, selected_features=[3, 4, 5, 6, 7, 8],
          imput_feature='AVG_DOWNHOLE_TEMPERATURE', target=2, test_idx=test_idx_dht,
          layers=[16, 8], dropout=0.0, epoch_num=30, test_ratio=0.1,
          verbose=0, display_plt=False, run_train=False)

MLP_imput(dataset, idx_to_del, selected_features=[3, 4, 5, 6, 7, 8],
          imput_feature='AVG_DOWNHOLE_PRESSURE', target=1, test_idx=test_idx_dhp,
          layers=[20, 10], dropout=0.0, epoch_num=80, test_ratio=0.1,
          verbose=0, display_plt=False, run_train=False)

MLP_imput(dataset, idx_to_del, selected_features=[1, 2, 4, 5, 6, 7, 8],
          imput_feature='ANNULUS_PRESS', target=0, test_idx=test_idx_ann,
          layers=[32, 16], dropout=0.0, epoch_num=300, test_ratio=0.1,
          verbose=0, display_plt=False, run_train=False)
...
display_plt = False
verbose=0
if os.path.isfile(os.path.join(dir, 'models', '%s_model.json' % (imput_feature))):
  f = open(os.path.join(dir, 'models', '%s_model.json' % (imput_feature)), 'r')
  loaded_json = f.read()
  f.close()
  model = model_from_json(loaded_json)
...
print(imput_feature, '\n', 'val set r2: ', r2_score(test_y, pred_y))
